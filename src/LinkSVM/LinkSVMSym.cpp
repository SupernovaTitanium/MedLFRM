...
std::set<std::pair<int, int> > sym_svm_links;
...
double LinkSVMSym::train(char *directory, Corpus *pC) {
  ...
  // test data accuracy
  // Document *pDoc = NULL;
  printf("\n\n LinkSVMSym: test accuracy: %.5f; AUC: %.4f\n\n", dTstAcc,
      dTstAUC);
...
double LinkSVMSym::e_step(Corpus *pC, double **var_nu, double **phi, int
batch_size) {
...
double LinkSVMSym::loss_aug_predict(Document *doc, Document *doc2, double *phi,
    double *phi2, const int &j) {
...
double LinkSVMSym::compute_disc_fscore(const int &y, Document *doc, Document
*doc2, double *phi, double *phi2) {
  double dScore = 0;
...
void LinkSVMSym::update_phi(Corpus *pC, Document *pDoc, double **phi, const int
&docId,
    double **var_nu) {
  //printf("id = %d\n", docId);
  clock_t start_time = clock();
  ...
  double dTrainTime = (double)(clock() - start_time)/CLOCKS_PER_SEC;
  ...
  for (int k = 0; k < m_nK; k++) {
    ...
    for (int i = 0, j = (randomMT()) % (pDoc->num_neighbors);
      ...
      if (jIx != docId) { // different entities
        loss_aug_predict(pDoc, pDoc2, phiPtr, phi[jIx], j);
        dVal += compute_mrgterm_right(pDoc, phi[jIx], j, k);
        if (sym_svm_links.find(make_pair(docId, jIx)) == sym_svm_links.end()) {
          ...
          mini_batch_svm_label[mini_batch_svm_size] =
              pDoc->linkGnd[j];
          sym_svm_links.insert(make_pair(docId, jIx));
          mini_batch_svm_size++;
        }
        ...
        loss_aug_predict(pDoc, pDoc2, phiPtr, phi[jIx], j);
        dVal += compute_mrgterm_self(pDoc, phi[jIx], j, k);
        if (sym_svm_links.find(make_pair(docId, docId)) ==
            sym_svm_links.end()) {
          mini_batch_svm_from[mini_batch_svm_size] = docId;
          mini_batch_svm_to[mini_batch_svm_size] = docId;
          mini_batch_svm_label[mini_batch_svm_size] =
              pDoc->linkGnd[j];
          sym_svm_links.insert(make_pair(docId, docId));
          mini_batch_svm_size++;
        }
...
double LinkSVMSym::compute_mrgterm_right(Document *doc, double *phi, const int
&j, const int &k) {
  double dval = 0;
...
double LinkSVMSym::compute_mrgterm_left(Document *doc, double *phi, const int
&j, const int &k) {
  double dval = 0;
  int gnd = doc->linkGnd[j];
...
double LinkSVMSym::compute_mrgterm_self(Document *doc, double *phi, const int
&j, const int &k) {
  double dval = 0;
  int gnd = doc->linkGnd[j];
...
void LinkSVMSym::update_nu(Corpus *pC, double **var_nu, double **phi, int
*batch, int batch_size, int iter) {
  //double *phiPtr = NULL;
  clock_t start_time = clock();
  // sum the expected number of features.
  memset(m_exp_feat_num_, 0, sizeof(double) * m_nK);
...
void LinkSVMSym::sample_init(Corpus *pC, double **phi, STRUCT_LEARN_PARM
*struct_parm) {
  char buff[512];
...
void LinkSVMSym::sample_init2(Corpus *pC, double **phi, STRUCT_LEARN_PARM
*struct_parm) {
...
void LinkSVMSym::set_init_param(STRUCT_LEARN_PARM *struct_parm, LEARN_PARM
*learn_parm,
...
void LinkSVMSym::get_index(const int &ix, int &rowIx, int &colIx) {
...
void LinkSVMSym::get_fvec(Document *pDoc1, Document *pDoc2, double *phi1,
    double *phi2, double *fvec) {
  int ix = 0;
  for (int m = 0; m < m_nK; m++) {// latent features.
    for (int n = 0; n < m; n++) {
...
void LinkSVMSym::learn_svm(Corpus *pC, double **phi, double *dMu, double eps,
  ...
  int s;
  double C, d, G;
  double t_alpha[m_nLabelNum * l];
  double t_w[m_nLabelNum * w_size];
  double fvec[w_size];
  ...
  for (int yi = 0; yi < m_nLabelNum; yi++) {
    ...
    for (int i = 0; i < m_nLatentFeatures; i++) {
      int wIx = nRefIx + i;
      get_index(i, rowIx, colIx);
      wPtr[rowIx][colIx] = wPtr[colIx][rowIx] = t_w[wIx];
    }
...
// deprecated
void LinkSVMSym::learn_svm_mini_batch(Corpus *pC, double **phi, double *dMu,
...
void LinkSVMSym::learn_svm_pegasos(Corpus *pC, double **phi, int svm_iter) {
  clock_t start_time = clock();
...
void LinkSVMSym::extract_train_links(Corpus *pC, int *from, int *to, int
*label, int l) {
  int id = 0;
  for (int d = 0; d < pC->num_docs; d++) {
    Document *pDoc = &(pC->docs[d]);
