...
LinkSVMSym::LinkSVMSym(Params *param) {
    _delay_nu = 1;
...
void LinkSVMSym::new_model(int num_docs, int num_train_links) {
    int i, j;
    m_nK = m_pParam->T;
    ...
    m_nLatentFeatures = m_nK * (m_nK + 1) / 2;
...
double LinkSVMSym::train(char *directory, Corpus *pC) {
    int d, k;
    ...
    double dBestTstAcc, dBestTstAUC, dBestTstF1, dBestTrAUC = 0;
    int nIt = 0;
    ...
    double dTrainTime = ((double) runtime_end - (double) runtime_start) / 100.0 - dTstTime;
    ...
    // test data accuracy
    Document *pDoc = NULL;
    printf("\n\n Logistic: test accuracy: %.5f; AUC: %.4f\n\n", dTstAcc, dTstAUC);
    ...
    fprintf(fileptr, "(K: %d; nu: %d, phi %d, k-nu: %f, k-phi: %f, F: %d; alpha: %.3f; C1: %.3f; C2: %.3f; Ell: %.3f):\n acc %.3f; f1: %.3f; auc: %.3f; fst_acc: %.3f; fst_f1: %.3f; fst_auc: %.3f; train: %.4f; test: %.4f\n",
            m_nK, m_pParam->STOCHASTIC_NU, m_pParam->STOCHASTIC_PHI, m_pParam->FORGETRATE_NU, m_pParam->FORGETRATE_PHI, m_pParam->NFOLDS, m_pParam->INITIAL_ALPHA, m_dC, m_pParam->INITIAL_C2, m_dDeltaEll,
...
*/
double LinkSVMSym::e_step(Corpus *pC, double **var_nu, double **phi) {
    Document *pDoc = NULL;
...
void LinkSVMSym::random_batch(int *target, int batch_size, int start, int end) {
    int len = end - start;
...
* perform inference on documents and update sufficient statistics
*/
double LinkSVMSym::e_step(Corpus *pC, double **var_nu, double **phi, int batch_size) {
    Document *pDoc = NULL;
 ...
 * compute lhood bound
 */
double LinkSVMSym::compute_lhood(Corpus *pC, double **phi, double **var_nu) {
    double lhood = 0;
    Document *pDoc = NULL;
...
double LinkSVMSym::loss_aug_predict(Document *doc, Document *doc2, double *phi, double *phi2, const int &j) {
...
double LinkSVMSym::get_2norm_w_sym() {
    double dRes = 0;
...
double LinkSVMSym::compute_disc_fscore(const int &y, Document *doc, Document *doc2, double *phi, double *phi2) {
    double dScore = 0;
...
void LinkSVMSym::random_init(Corpus *pC, double **phi) {
...
void LinkSVMSym::random_wvar_nu() {
...
void LinkSVMSym::update_phi(Corpus *pC, Document *pDoc, double **phi, const int &docId,
        double **var_nu) {
    double *wMeanPtr = NULL, dVal = 0;

    //FILE *fptr = fopen("phi_intermediate.txt", "a");
    double *phiPtr = phi[docId];
    double *yDistPtr = m_dYDist[docId];
...
void LinkSVMSym::update_phi(Corpus *pC, Document *pDoc, double **phi, const int &docId,
    ...
    int _iter = phi_iter;
    while (_iter--) {
        random_batch(sto, batch_size, 0, pDoc->num_neighbors);
        for (int k = 0; k < m_nK; k++) {
            //wMeanPtr = m_dW[k];

            dVal = 0;
            for (int j = 0; j < batch_size; j++) {
                int jIx = pDoc->neighbors[sto[j]];
                if (!pDoc->bTrain[j]) continue;

                Document *pDoc2 = &(pC->docs[jIx]);
                if (jIx != docId) { // different entities
                    loss_aug_predict(pDoc, pDoc2, phiPtr, phi[jIx], sto[j]);
                    dVal += compute_mrgterm_right(pDoc, phi[jIx], sto[j], k);
                    ...
                    loss_aug_predict(pDoc, pDoc2, phiPtr, phi[jIx], sto[j]);
                    dVal += compute_mrgterm_self(pDoc, phi[jIx], sto[j], k);
                }
            }
            dVal *= pDoc->num_neighbors / batch_size;
            dVal += m_dNuExpSum[k] - m_dNuExpProd[k];
            double newPhi = safe_logist(dVal);
            // step size at iteration ``iter``
            double rou = step_size(iter * phi_iter + _iter, _delay_phi, _forgetting_rate_phi);
...
double LinkSVMSym::compute_mrgterm_right(Document *doc, double *phi, const int &j, const int &k) {
    double dval = 0;
    int gnd = doc->linkGnd[j];
    int lossAug = doc->linkLossAug[j];

    if (gnd != lossAug) {
        dval = (dotprod(m_dW[gnd][k], phi, m_nK) - dotprod(m_dW[lossAug][k], phi, m_nK));
...
double LinkSVMSym::compute_mrgterm_left(Document *doc, double *phi, const int &j, const int &k) {
    double dval = 0;
    int gnd = doc->linkGnd[j];
    int lossAug = doc->linkLossAug[j];
...
double LinkSVMSym::compute_mrgterm_self(Document *doc, double *phi, const int &j, const int &k) {
    double dval = 0;
    int gnd = doc->linkGnd[j];
    int lossAug = doc->linkLossAug[j];
    ...
    if (gnd != lossAug) {
        dval = (dotprod(m_dW[gnd][k], phi, m_nK) - dotprod(m_dW[lossAug][k], phi, m_nK));
        dval += (m_dW[gnd][k][k] - m_dW[lossAug][k][k]) * (1 - phi[k]);
        if (gnd == 0) dval *= m_pParam->INITIAL_C1;
        else dval *= m_pParam->INITIAL_C2;
    }
...
void LinkSVMSym::update_nu(Corpus *pC, double **var_nu, double **phi) {
    double *phiPtr = NULL;
...
void LinkSVMSym::update_nu(Corpus *pC, double **var_nu, double **phi, int *batch, int batch_size, int iter) {
    double *phiPtr = NULL;
...
void LinkSVMSym::compute_nu_stat(double **var_nu) {
...
double LinkSVMSym::compute_nu_exp_prod(const int &k) {
...
void LinkSVMSym::update_ydist(Corpus *pC, double **phi) {
    for (int d = 0; d < pC->num_docs; d++) {
        Document *pDoc = &(pC->docs[d]);
        double *yDistPtr = m_dYDist[d];
        double *phiPtr = phi[d];
        ...
        for (int i = 0; i < pDoc->num_neighbors; i++) {
            ...
            double dval = compute_disc_fscore(1, pDoc, &(pC->docs[jIx]), phiPtr, phi[jIx]) - compute_disc_fscore(0, pDoc, &(pC->docs[jIx]), phiPtr, phi[jIx]);
...
void LinkSVMSym::update_ydist_tr(Corpus *pC, double **phi) {
    for (int d = 0; d < pC->num_docs; d++) {
        Document *pDoc = &(pC->docs[d]);
        double *yDistPtr = m_dYDist[d];
        double *phiPtr = phi[d];
...
void LinkSVMSym::sample_init(Corpus *pC, double **phi, STRUCT_LEARN_PARM *struct_parm) {
    char buff[512];
...
void LinkSVMSym::sample_init2(Corpus *pC, double **phi, STRUCT_LEARN_PARM *struct_parm) {
    ...
    for (int d = 0; d < pC->num_docs; d++) {
        ...
        for (int k = 0; k < pDoc->num_neighbors; k++) {
            ...
            get_fvec_sym(pDoc, &(pC->docs[jIx]), phi[d], phi[jIx], fvec_);
...
void LinkSVMSym::set_init_param(STRUCT_LEARN_PARM *struct_parm, LEARN_PARM *learn_parm,
...
void LinkSVMSym::learn_svm(Corpus *pC, double **phi, double *dMu) {
    ...
    */
    ...
    for (int y = 0; y < m_nLabelNum; y++) {
        ...
        for (int i = 0; i < m_nLatentFeatures; i++) {
            int wIx = nRefIx + i;
            get_index_sym(i, rowIx, colIx);
            wPtr[rowIx][colIx] = structmodel.w[wIx + 1];
            wPtr[colIx][rowIx] = wPtr[rowIx][colIx];
        }
        double *wOrgPtr = m_dOrgW[y];
        for (int i = 0; i < m_nOrgFeatures; i++) {
...
void LinkSVMSym::get_index_sym(const int &ix, int &rowIx, int &colIx) {
    for (rowIx = 0; rowIx < m_nK; rowIx++) {
        if ((rowIx + 2) * (rowIx + 1) > ix * 2) break;
    }
    colIx = ix - (rowIx * (rowIx + 1) / 2);
}

void LinkSVMSym::outputLowDimData(char *filename, Corpus *pC, double **phi) {
    Document *pDoc = NULL;
    ...
    for (int d = 0; d < pC->num_docs; d++) {
        ...
        for (int i = 0; i < pDoc->num_neighbors; i++) {
            ...
            get_fvec_sym(pDoc, &(pC->docs[jIx]), phiPtr, phiPtr2, fvec_);
            for (int m = 0; m < m_nSVMFeature; m++) {
...
void LinkSVMSym::get_fvec_sym(Document *pDoc1, Document *pDoc2, double *phi1, double *phi2, double *fvec) {
    int ix = 0;
    for (int m = 0; m < m_nK; m++) {// latent features.
        for (int n = 0; n < m; n++) {
            fvec[ix] = phi1[m] * phi2[n] + phi1[n] * phi2[m];
            ix++;
        }
        fvec[ix] = phi1[m] * phi2[m];
        ix++;
    }
...
void LinkSVMSym::save_model(char *model_root, Corpus *pC, double **phi) {
    char filename[512];
    FILE *fileptr;
    int i, j;
...
void LinkSVMSym::load_model(char *model_root) {
    char filename[512];
    FILE *fileptr;
...
double LinkSVMSym::get_test_acc(Corpus *pC, double **phi) {
    for (int i = 0; i < 2; i++) {
        dAVec[i] = 0;
        dBVec[i] = 0;
        dCVec[i] = 0;
...
void LinkSVMSym::predict(Document *doc, const int &i, double *yDist) {
    doc->linkTest[i] = (yDist[i] >= 0.5) ? 1 : 0;
}

// compute the AUC score
double LinkSVMSym::compute_auc(Corpus *pC, double **yDist) {
    vector<int> gndLabel;
    ...
    for (int i = 0; i < nIx; i++) {
        for (int j = nIx - 1; j > i; j--) {
            if (predScore[j] > predScore[j - 1]) {
                double dVal = predScore[j - 1];
                predScore[j - 1] = predScore[j];
                predScore[j] = dVal;

                int label = gndLabel[j - 1];
                gndLabel[j - 1] = gndLabel[j];
                gndLabel[j] = label;
            }
        }
    }
    ...
    //vector<double> truePos(nIx, 0);
    //vector<double> falsePos(nIx, 0);
    int nAccPos = 0;
    m_nTstLinks = nIx;
...
// compute the AUC score
double LinkSVMSym::compute_auc_tr(Corpus *pC, double **yDist) {
    vector<int> gndLabel;
    vector<double> predScore;
...
*/
double LinkSVMSym::save_prediction(char *filename, Corpus *pC, double **phi) {
