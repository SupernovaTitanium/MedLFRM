...
void LinkSVM::new_model(int num_docs, int num_train_links) {
    m_nK = m_pParam->T;
...
double LinkSVM::train(char *directory, Corpus *pC) {
    ...
    // test data accuracy
    // Document *pDoc = NULL;
    printf("\n\n LinkSVM: test accuracy: %.5f; AUC: %.4f\n\n", dTstAcc, dTstAUC);
...
void LinkSVM::random_batch(int *target, int batch_size, int start, int end) {
    int len = end - start;
 ...
 * compute lhood bound
 */
double LinkSVM::compute_lhood(Corpus *pC, double **phi, double **var_nu) {
    double lhood = 0;
    Document *pDoc = NULL;
...
double LinkSVM::loss_aug_predict(Document *doc, Document *doc2, double *phi, double *phi2, const int &j) {
...
double LinkSVM::compute_disc_fscore(const int &y, Document *doc, Document *doc2, double *phi, double *phi2) {
    double dScore = 0;
...
void LinkSVM::random_init(Corpus *pC, double **phi) {
...
double LinkSVM::compute_mrgterm_right(Document *doc, double *phi, const int &j, const int &k) {
    double dval = 0;
...
double LinkSVM::compute_mrgterm_left(Document *doc, double *phi, const int &j, const int &k) {
    double dval = 0;
    int gnd = doc->linkGnd[j];
...
double LinkSVM::compute_mrgterm_self(Document *doc, double *phi, const int &j, const int &k) {
    double dval = 0;
    int gnd = doc->linkGnd[j];
...
void LinkSVM::compute_nu_stat(double **var_nu) {
...
double LinkSVM::compute_nu_exp_prod(const int &k) {
...
void LinkSVM::update_ydist(Corpus *pC, double **phi) {
    for (int d = 0; d < pC->num_docs; d++) {
        Document *pDoc = &(pC->docs[d]);
        double *yDistPtr = m_dYDist[d];
        double *phiPtr = phi[d];
...
void LinkSVM::update_ydist_tr(Corpus *pC, double **phi) {
    for (int d = 0; d < pC->num_docs; d++) {
        Document *pDoc = &(pC->docs[d]);
        double *yDistPtr = m_dYDist[d];
        double *phiPtr = phi[d];
...
void LinkSVM::sample_init(Corpus *pC, double **phi, STRUCT_LEARN_PARM *struct_parm) {
    char buff[512];
...
void LinkSVM::sample_init2(Corpus *pC, double **phi, STRUCT_LEARN_PARM *struct_parm) {
...
void LinkSVM::set_init_param(STRUCT_LEARN_PARM *struct_parm, LEARN_PARM *learn_parm,
...
void LinkSVM::outputLowDimData(char *filename, Corpus *pC, double **phi) {
    Document *pDoc = NULL;
...
void LinkSVM::get_fvec(Document *pDoc1, Document *pDoc2, double *phi1, double *phi2, double *fvec) {
    int ix = 0;
    for (int m = 0; m < m_nK; m++) {// latent features.
        for (int n = 0; n < m_nK; n++) {
...
void LinkSVM::save_model(char *model_root, Corpus *pC, double **phi) {
    char filename[512];
    FILE *fileptr;
...
void LinkSVM::load_model(char *model_root) {
    char filename[512];
    FILE *fileptr;
...
double LinkSVM::get_test_acc(Corpus *pC, double **phi) {
    for (int i = 0; i < 2; i++) {
        dAVec[i] = 0;
        dBVec[i] = 0;
        dCVec[i] = 0;
...
void LinkSVM::predict(Document *doc, const int &i, double *yDist) {
    doc->linkTest[i] = (yDist[i] >= 0.5) ? 1 : 0;
}

// compute the AUC score
double LinkSVM::compute_auc(Corpus *pC, double **yDist) {
    //vector<int> gndLabel;
...
// compute the AUC score
double LinkSVM::compute_auc_tr(Corpus *pC, double **yDist) {
    vector<std::pair<double, int> > ScrLbl;
...
*/
double LinkSVM::save_prediction(char *filename, Corpus *pC, double **phi) {
...
void LinkSVM::extract_train_links(Corpus *pC, int *from, int *to, int *label, int l) {
    int id = 0;
    for (int d = 0; d < pC->num_docs; d++) {
        Document *pDoc = &(pC->docs[d]);
        for (int k = 0; k < pDoc->num_neighbors; k++) {
