...
BayesLinkSVM::BayesLinkSVM(void) {
  _delay_nu = 1;
...
BayesLinkSVM::BayesLinkSVM(Params *param) {
  _delay_nu = 1;
...
void BayesLinkSVM::new_model(int num_docs, int num_train_links) {
  m_nK = m_pParam->T;
  ...
  m_pNormGammaPrior = new NormalGammaPrior(0, 1, 2, 1);
  m_dWExp_ = new double[m_nSVMFeature * m_nLabelNum];

}

void BayesLinkSVM::free_model() {
...
double BayesLinkSVM::train(char *directory, Corpus *pC) {
  int d;
  ...
  // test data accuracy
  // Document *pDoc = NULL;
  printf("\n\n BayesLinkSVM: test accuracy: %.5f; AUC: %.4f\n\n", dTstAcc, dTstAUC);
...
double BayesLinkSVM::update_hyperparam() {
  double dMu = 0, dTau = 0;
...
*/
double BayesLinkSVM::e_step(Corpus *pC, double **var_nu, double **phi) {
  clock_t start_time = clock();
  Document *pDoc = NULL;
...
void BayesLinkSVM::random_batch(int *target, int batch_size, int start, int end) {
  int len = end - start;
...
double BayesLinkSVM::e_step(Corpus *pC, double **var_nu, double **phi, int batch_size) {
 ...
 * compute lhood bound
 */
double BayesLinkSVM::compute_lhood(Corpus *pC, double **phi, double **var_nu) {
  double lhood = 0;
  Document *pDoc = NULL;
...
double BayesLinkSVM::loss_aug_predict(Document *doc, Document *doc2, double *phi, double *phi2, const int &j) {
...
double BayesLinkSVM::get_2norm_w() {
  double dRes = 0;
...
double BayesLinkSVM::compute_disc_fscore(const int &y, Document *doc, Document *doc2, double *phi, double *phi2) {
  double dScore = 0;
...
void BayesLinkSVM::random_init(Corpus *pC, double **phi) {
...
void BayesLinkSVM::random_wvar_nu() {
...
void BayesLinkSVM::update_phi(Corpus *pC, Document *pDoc, double **phi, const int &docId,
    double **var_nu) {
  //printf("id = %d\n", docId);
  clock_t start_time = clock();
...
double BayesLinkSVM::compute_mrgterm_right(Document *doc, double *phi, const int &j, const int &k) {
  double dval = 0;
...
double BayesLinkSVM::compute_mrgterm_left(Document *doc, double *phi, const int &j, const int &k) {
  double dval = 0;
  int gnd = doc->linkGnd[j];
...
double BayesLinkSVM::compute_mrgterm_self(Document *doc, double *phi, const int &j, const int &k) {
  double dval = 0;
  int gnd = doc->linkGnd[j];
...
void BayesLinkSVM::update_nu(Corpus *pC, double **var_nu, double **phi) {
  // double *phiPtr = NULL;
  clock_t start_time = clock();
...
void BayesLinkSVM::update_nu(Corpus *pC, double **var_nu, double **phi, int *batch, int batch_size, int iter) {
  //double *phiPtr = NULL;
  clock_t start_time = clock();
  // sum the expected number of features.
  memset(m_exp_feat_num_, 0, sizeof(double) * m_nK);
...
void BayesLinkSVM::compute_nu_stat(double **var_nu) {
...
double BayesLinkSVM::compute_nu_exp_prod(const int &k) {
...
void BayesLinkSVM::update_ydist(Corpus *pC, double **phi) {
  for (int d = 0; d < pC->num_docs; d++) {
    Document *pDoc = &(pC->docs[d]);
    double *yDistPtr = m_dYDist[d];
    double *phiPtr = phi[d];
...
void BayesLinkSVM::update_ydist_tr(Corpus *pC, double **phi) {
  for (int d = 0; d < pC->num_docs; d++) {
    Document *pDoc = &(pC->docs[d]);
    double *yDistPtr = m_dYDist[d];
    double *phiPtr = phi[d];
...
void BayesLinkSVM::sample_init(Corpus *pC, double **phi, STRUCT_LEARN_PARM *struct_parm) {
  char buff[512];
...
void BayesLinkSVM::sample_init2(Corpus *pC, double **phi, STRUCT_LEARN_PARM *struct_parm) {
...
void BayesLinkSVM::set_init_param(STRUCT_LEARN_PARM *struct_parm, LEARN_PARM *learn_parm,
...
void BayesLinkSVM::learn_svm(Corpus *pC, double **phi, double *dMu) {
  clock_t start_time = clock();
...
void BayesLinkSVM::get_index(const int &ix, int &rowIx, int &colIx) {
  if (m_pParam->SYMMETRIC == 1) {
...
void BayesLinkSVM::outputLowDimData(char *filename, Corpus *pC, double **phi) {
  Document *pDoc = NULL;
...
void BayesLinkSVM::get_fvec(Document *pDoc1, Document *pDoc2, double *phi1, double *phi2, double *fvec) {
  int ix = 0;
  for (int m = 0; m < m_nK; m++) {// latent features.
    for (int n = 0; n < m_nK; n++) {
      fvec[ix] = phi1[m] * phi2[n];
      ix++;
    }
  }
...
void BayesLinkSVM::save_model(char *model_root, Corpus *pC, double **phi) {
  char filename[512];
  FILE *fileptr;
...
void BayesLinkSVM::load_model(char *model_root) {
  char filename[512];
  FILE *fileptr;
...
double BayesLinkSVM::get_test_acc(Corpus *pC, double **phi) {
  for (int i = 0; i < 2; i++) {
    dAVec[i] = 0;
    dBVec[i] = 0;
    dCVec[i] = 0;
...
void BayesLinkSVM::predict(Document *doc, const int &i, double *yDist) {
  doc->linkTest[i] = (yDist[i] >= 0.5) ? 1 : 0;
}

// compute the AUC score
double BayesLinkSVM::compute_auc(Corpus *pC, double **yDist) {
  //vector<int> gndLabel;
...
// compute the AUC score
double BayesLinkSVM::compute_auc_tr(Corpus *pC, double **yDist) {
  vector<std::pair<double, int> > ScrLbl;
...
*/
double BayesLinkSVM::save_prediction(char *filename, Corpus *pC, double **phi) {
...
void BayesLinkSVM::learn_svm(Corpus *pC, double **phi, double *dMu, double eps,
  ...
  for (int yi = 0; yi < m_nLabelNum; yi++) {
    ...
    for (int i = 0; i < m_nLatentFeatures; i++) {
      int wIx = nRefIx + i;
      get_index(i, rowIx, colIx);
      wPtr[rowIx][colIx] = t_w[wIx] + m_dMu0;
    }
...
void BayesLinkSVM::learn_svm_mini_batch(Corpus *pC, double **phi, double *dMu,
...
void BayesLinkSVM::learn_svm_pegasos(Corpus *pC, double **phi, int svm_iter) {
  clock_t start_time = clock();
...
void BayesLinkSVM::extract_train_links(Corpus *pC, int *from, int *to, int *label, int l) {
  int id = 0;
  for (int d = 0; d < pC->num_docs; d++) {
    Document *pDoc = &(pC->docs[d]);
    for (int k = 0; k < pDoc->num_neighbors; k++) {
