...
BayesLinkSVMSym::BayesLinkSVMSym(void) {
  _delay_nu = 1;
  ...
  phi_iter = 1;
  m_pNormGammaPrior = NULL;
}

BayesLinkSVMSym::BayesLinkSVMSym(Params *param) {
  _delay_nu = 1;
...
void BayesLinkSVMSym::new_model(int num_docs, int num_train_links) {
  m_nK = m_pParam->T;
  ...
  m_pNormGammaPrior = new NormalGammaPrior(0, 1, 2, 2);
  m_dWExp_ = new double[m_nSVMFeature * m_nLabelNum];

}

void BayesLinkSVMSym::free_model() {
  ...
  delete []m_dWExp_;
  delete m_pNormGammaPrior;
}

double BayesLinkSVMSym::train(char *directory, Corpus *pC) {
  int d;
  ...
  while (/*((converged > m_pParam->EM_CONVERGED)
    ...
    double dPriorKL = update_hyperparam();

    double dobj = lhood + dPriorKL;
    ...
    dTstTime += (clock() - start_test_time) / CLOCKS_PER_SEC;
    ...
    fprintf(likelihood_file, "%.10f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%5.5e\t%"
            ".10f\t.10f\n",
        dobj, dPriorKL, dTstAUC, m_dF1Score, dTrAUC, converged, m_dMu0,
        m_dC);
  ...
  double dTrainTime = (clock() - start_time) / CLOCKS_PER_SEC - dTstTime;
  ...
  // test data accuracy
  // Document *pDoc = NULL;
  printf("\n\n BayesLinkSVMSym: test accuracy: %.5f; AUC: %.4f\n\n", dTstAcc, dTstAUC);
...
double BayesLinkSVMSym::update_hyperparam() {
  double dMu = 0, dTau = 0;
  int nDim = m_nSVMFeature * m_nLabelNum;
  int ix = 0;
  for (int y = 0; y < m_nLabelNum; y++) {
    for (int k = 0; k < m_nK; k++) {
      for (int m = 0; m <= k; m++) {
        m_dWExp_[ix] = m_dW[y][k][m];
        ix++;
      }
    }
  }
  for (int y = 0; y < m_nLabelNum; y++) {
    for (int i = 0; i < m_nOrgFeatures; i++) {
      m_dWExp_[ix] = m_dOrgW[y][i];
      ix++;
    }
  }
  nDim = ix;
  double dPriorKL = m_pNormGammaPrior->mean_field(dMu, dTau, m_dWExp_, nDim);
  m_dMu0 = dMu;
  m_dC = 1 / dTau;
  return dPriorKL;
}
...
*/
double BayesLinkSVMSym::e_step(Corpus *pC, double **var_nu, double **phi) {
  clock_t start_time = clock();
  Document *pDoc = NULL;
  ...
  double dTrainTime = (clock() - start_time) / CLOCKS_PER_SEC;
  running_time_for_vi += dTrainTime;
  return lhood;
}

void BayesLinkSVMSym::random_batch(int *target, int batch_size, int start, int end) {
  int len = end - start;
...
double BayesLinkSVMSym::e_step(Corpus *pC, double **var_nu, double **phi, int batch_size) {
 ...
 * compute lhood bound
 */
double BayesLinkSVMSym::compute_lhood(Corpus *pC, double **phi, double **var_nu) {
  double lhood = 0;
  Document *pDoc = NULL;
  ...
  for (int i = 0; i < pC->num_docs; i++) {
    ...
    for (int j = 0; j < pDoc->num_neighbors; j++) {
      ...
      if (pDoc->linkGnd[j] == 0)
        dHingeLoss += loss_aug_predict(pDoc, &(pC->docs[jIx]), phi[i],
            phi[jIx], j);// * m_pParam->INITIAL_C1;
      else
        dHingeLoss += loss_aug_predict(pDoc, &(pC->docs[jIx]), phi[i],
            phi[jIx], j);// * m_pParam->INITIAL_C2;
    }
  }
  lhood += dHingeLoss;
...
double BayesLinkSVMSym::loss_aug_predict(Document *doc, Document *doc2, double *phi, double *phi2, const int &j) {
...
double BayesLinkSVMSym::get_2norm_w() {
  double dRes = 0;
  ...
  for (int y = 0; y < m_nLabelNum; y++) {
    ...
    for (int i = 0; i < m_nK; i++) {
      for (int j = 0; j <= i; j++) {
        dRes += (wPtr[i][j] - m_dMu0)* (wPtr[i][j] - m_dMu0);
      }
    }
    double *wOrgPtr = m_dOrgW[y];
    for (int i = 0; i < m_nOrgFeatures; i++) {
      dRes += (wOrgPtr[i] - m_dMu0) * (wOrgPtr[i] - m_dMu0);
    }
  }
...
double BayesLinkSVMSym::compute_disc_fscore(const int &y, Document *doc, Document *doc2, double *phi, double *phi2) {
  double dScore = 0;
...
void BayesLinkSVMSym::random_init(Corpus *pC, double **phi) {
...
void BayesLinkSVMSym::random_wvar_nu() {
...
void BayesLinkSVMSym::update_phi(Corpus *pC, Document *pDoc, double **phi, const int &docId,
    double **var_nu) {
  //printf("id = %d\n", docId);
  clock_t start_time = clock();
...
double BayesLinkSVMSym::compute_mrgterm_right(Document *doc, double *phi, const int &j, const int &k) {
  double dval = 0;
...
double BayesLinkSVMSym::compute_mrgterm_left(Document *doc, double *phi, const int &j, const int &k) {
  double dval = 0;
  int gnd = doc->linkGnd[j];
...
double BayesLinkSVMSym::compute_mrgterm_self(Document *doc, double *phi, const int &j, const int &k) {
  double dval = 0;
  int gnd = doc->linkGnd[j];
...
void BayesLinkSVMSym::update_nu(Corpus *pC, double **var_nu, double **phi) {
  // double *phiPtr = NULL;
  clock_t start_time = clock();
...
void BayesLinkSVMSym::update_nu(Corpus *pC, double **var_nu, double **phi, int *batch, int batch_size, int iter) {
  //double *phiPtr = NULL;
  clock_t start_time = clock();
  // sum the expected number of features.
  memset(m_exp_feat_num_, 0, sizeof(double) * m_nK);
...
void BayesLinkSVMSym::compute_nu_stat(double **var_nu) {
...
double BayesLinkSVMSym::compute_nu_exp_prod(const int &k) {
...
void BayesLinkSVMSym::update_ydist(Corpus *pC, double **phi) {
  for (int d = 0; d < pC->num_docs; d++) {
    Document *pDoc = &(pC->docs[d]);
    double *yDistPtr = m_dYDist[d];
    double *phiPtr = phi[d];
...
void BayesLinkSVMSym::update_ydist_tr(Corpus *pC, double **phi) {
  for (int d = 0; d < pC->num_docs; d++) {
    Document *pDoc = &(pC->docs[d]);
    double *yDistPtr = m_dYDist[d];
    double *phiPtr = phi[d];
...
void BayesLinkSVMSym::sample_init(Corpus *pC, double **phi, STRUCT_LEARN_PARM *struct_parm) {
  char buff[512];
...
void BayesLinkSVMSym::sample_init2(Corpus *pC, double **phi, STRUCT_LEARN_PARM *struct_parm) {
...
void BayesLinkSVMSym::set_init_param(STRUCT_LEARN_PARM *struct_parm, LEARN_PARM *learn_parm,
...
void BayesLinkSVMSym::learn_svm(Corpus *pC, double **phi, double *dMu) {
  clock_t start_time = clock();
...
void BayesLinkSVMSym::get_index(const int &ix, int &rowIx, int &colIx) {
...
void BayesLinkSVMSym::outputLowDimData(char *filename, Corpus *pC, double **phi) {
  Document *pDoc = NULL;
...
void BayesLinkSVMSym::get_fvec(Document *pDoc1, Document *pDoc2, double *phi1, double *phi2, double *fvec) {
  int ix = 0;
  for (int m = 0; m < m_nK; m++) {// latent features.
    for (int n = 0; n < m; n++) {
...
void BayesLinkSVMSym::save_model(char *model_root, Corpus *pC, double **phi) {
  char filename[512];
  FILE *fileptr;
...
void BayesLinkSVMSym::load_model(char *model_root) {
  char filename[512];
  FILE *fileptr;
...
double BayesLinkSVMSym::get_test_acc(Corpus *pC, double **phi) {
  for (int i = 0; i < 2; i++) {
    dAVec[i] = 0;
    dBVec[i] = 0;
    dCVec[i] = 0;
...
void BayesLinkSVMSym::predict(Document *doc, const int &i, double *yDist) {
  doc->linkTest[i] = (yDist[i] >= 0.5) ? 1 : 0;
}

// compute the AUC score
double BayesLinkSVMSym::compute_auc(Corpus *pC, double **yDist) {
  //vector<int> gndLabel;
...
// compute the AUC score
double BayesLinkSVMSym::compute_auc_tr(Corpus *pC, double **yDist) {
  vector<std::pair<double, int> > ScrLbl;
...
*/
double BayesLinkSVMSym::save_prediction(char *filename, Corpus *pC, double **phi) {
...
void BayesLinkSVMSym::learn_svm(Corpus *pC, double **phi, double *dMu, double eps,
  ...
  for (int yi = 0; yi < m_nLabelNum; yi++) {
    ...
    for (int i = 0; i < m_nLatentFeatures; i++) {
      int wIx = nRefIx + i;
      get_index(i, rowIx, colIx);
      wPtr[rowIx][colIx] = wPtr[colIx][rowIx] = t_w[wIx] + m_dMu0;
    }
    double *wOrgPtr = m_dOrgW[yi];
    for (int i = 0; i < m_nOrgFeatures; i++) {
      int wIx = nRefIx + m_nLatentFeatures + i;
      wOrgPtr[i] = t_w[wIx] + m_dMu0;
    }
  }
  delete fvec;
  double dTrainTime = (clock() - start_time) / CLOCKS_PER_SEC;
...
void BayesLinkSVMSym::learn_svm_mini_batch(Corpus *pC, double **phi, double *dMu,
...
void BayesLinkSVMSym::learn_svm_pegasos(Corpus *pC, double **phi, int svm_iter) {
  clock_t start_time = clock();
...
void BayesLinkSVMSym::extract_train_links(Corpus *pC, int *from, int *to, int *label, int l) {
  int id = 0;
  for (int d = 0; d < pC->num_docs; d++) {
    Document *pDoc = &(pC->docs[d]);
    for (int k = 0; k < pDoc->num_neighbors; k++) {
